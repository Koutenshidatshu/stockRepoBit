...
class CryptoResponseTests: XCTestCase {
    ...
    func testParseWithEmptyData() {
        ...
        let cryptoResponse = CryptoRequester(requestApi: .just(emptyData.data(using: .utf8)!))
        var result: CryptoResponse?
        
        
        cryptoResponse.request()
    ...
    func testParseWithValidData() {
        let disposeBag = DisposeBag()
        let cryptoResponse = CryptoRequester(requestApi:
        ...
        XCTAssertGreaterThan(result!.blockChainData.count, 1)
        XCTAssertEqual(result!.blockChainData.first?.coinInfo.name, "BTC")
    ...
    func testWithInvalidData() {
        ...
        var capturedError : CryptoRequester.RequestError?
        
        cryptoResponse.request()
            .subscribe(onError: { capturedError = ($0 as? CryptoRequester.RequestError) })
            .disposed(by: disposeBag)
        XCTAssertEqual(capturedError, CryptoRequester.RequestError.parse)
    }
